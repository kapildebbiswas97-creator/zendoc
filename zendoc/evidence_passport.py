...
The current ai_interactions table stores model/provider/risk/audit metadata but
not source-level citations. This module makes that limitation visible instead
of reconstructing or fabricating evidence after the fact.
"""
...
def _passport(row):
    ...
    return {
        ...
        "patient_data_used": {
            ...
            "notice": "The historical interaction log does not prove that any additional Health Memory context was used, so ZENDOC does not claim it here.",
        },
        "source_level_evidence": {
            "status": "not_recorded",
            "sources": [],
            "notice": "This historical interaction record does not contain source-level citations. ZENDOC will not invent citations after the fact.",
        },
        "uncertainty": {
        ...
        "human_review": {
            "state": "not_recorded",
            "notice": "The interaction log does not record a clinician review state unless another workflow explicitly stores one.",
        },
        "policy": {
