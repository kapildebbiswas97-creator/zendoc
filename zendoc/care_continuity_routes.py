...
def _journey_cards(user):
    journeys = list_patient_journeys(user, limit=12)
    cards = []
    for journey in journeys:
        try:
            actions = list_actions(user, journey["id"])
        except (PermissionError, LookupError, TypeError, ValueError):
            actions = []
        cards.append({
            "journey": journey,
            "actions": actions,
            "open_actions": [item for item in actions if item.get("status") not in {"COMPLETED", "BLOCKED", "CANCELLED"}],
            "completed_actions": [item for item in actions if item.get("status") == "COMPLETED"],
            "outcome_count": sum(len(item.get("outcomes") or []) for item in actions),
        })
    return cards
...
def care_continuity_page():
    _patient_only()
    reason = request.form.get("reason_for_visit", "") if request.method == "POST" else ""
    questions = request.form.get("questions", "") if request.method == "POST" else ""
    packet = build_clinician_handoff_packet(g.user, reason_for_visit=reason, questions=questions)
    passports = list_evidence_passports(g.user, limit=12)
    cards = _journey_cards(g.user)
