...
def _overpass_query(category, latitude, longitude, radius_metres):
    ...
    for key, value in _filters_for(category):
        clauses.append(f'nwr(around:{radius_metres},{latitude:.6f},{longitude:.6f})["{key}"="{value}"];')
...
def _infer_category(tags, requested):
    ...
    if amenity == "clinic" or healthcare == "clinic": return "clinic"
    if amenity == "doctors" or healthcare == "doctor": return "doctor"
    if healthcare == "laboratory" or amenity == "laboratory": return "diagnostic_centre"
    if healthcare in {"centre", "health_centre"}: return "health_centre"
    if healthcare == "nursing_home" or amenity == "nursing_home": return "nursing_home"
    if healthcare == "blood_bank" or amenity == "blood_bank": return "blood_bank"
...
def _address_from_tags(tags):
    ...
    if direct:
        return direct
    parts = []
    street = " ".join(part for part in (str(tags.get("addr:housenumber") or "").strip(), str(tags.get("addr:street") or "").strip()) if part).strip()
    if street:
        parts.append(street)
...
class OverpassHealthcareProvider:
    ...
    def search(self, query):
        ...
        try:
            ...
            for element in elements:
                ...
                seen.add(key)
                results.append(item)
            result = {"available": True, "results": results[:80], "message": None if results else "OpenStreetMap returned no tagged healthcare POIs near this location.", "source": self.source, "center": {"latitude": latitude, "longitude": longitude}}
            _cache_set(cache_key, result)
            return result
        except (urllib.error.HTTPError, urllib.error.URLError, TimeoutError, ValueError, json.JSONDecodeError):
            return {"available": False, "results": [], "message": "OpenStreetMap nearby healthcare discovery is temporarily unavailable. No listings were fabricated.", "source": self.source}
    ...
    def _post_overpass(self, query_text):
        ...
        request = urllib.request.Request(self.overpass_url, data=data, headers={"Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded", "User-Agent": USER_AGENT}, method="POST")
        with urllib.request.urlopen(request, timeout=self.timeout_seconds) as response:
