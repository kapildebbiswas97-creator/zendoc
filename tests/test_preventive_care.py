...
def test_patient_can_create_list_and_complete_recorded_preventive_plan(tmp_path):
    ...
    listed = client.get("/api/v1/preventive-care/plans?status=ACTIVE")
    assert listed.status_code == 200
    assert [item["plan_uid"] for item in listed.get_json()["plans"]] == [plan["plan_uid"]]

    completed = client.post(f"/api/v1/preventive-care/plans/{plan['id']}/complete")
...
def test_preventive_plan_blocks_cross_patient_idor(tmp_path):
    ...
    login_web(client, "patient", second_email)
    created = client.post(
        "/api/v1/preventive-care/plans",
    ...
    denied = client.get(f"/api/v1/preventive-care/plans?patient_id={second_id}")
...
def test_preventive_plan_requires_authentication_and_valid_category(tmp_path):
    ...
    register_web(client, "patient", "preventive-validation@example.com")
    login_web(client, "patient", "preventive-validation@example.com")
    invalid = client.post(
        "/api/v1/preventive-care/plans",
