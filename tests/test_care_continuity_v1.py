...
def test_patient_care_continuity_page_is_available(tmp_path):
    ...
    assert "does not automatically send" in body
...
def test_clinician_handoff_is_descriptive_and_not_auto_shared(tmp_path):
    ...
    response = client.get(
        ...
        query_string={
            ...
            "question": ["What could I ask about next steps?", "Do I need follow-up?"],
        },
    )
    assert response.status_code == 200
    packet = response.get_json()["handoff_packet"]

    assert packet["reason_for_visit"] == "Persistent back pain for one week"
    ...
    assert packet["questions_for_clinician"] == [
        "What could I ask about next steps?",
        "Do I need follow-up?",
    ]
...
def test_evidence_passport_never_invents_historical_sources(tmp_path):
    ...
    with app.app_context():
        ...
        cursor = get_db().execute(
            ...
            (
                ...
                "symptoms",
                "I have a headache",
                "Seek appropriate care if severe or worsening.",
                "low",
                "zendoc-test-model",
                "test-provider",
                0,
    ...
    assert passport["generation"]["provider"] == "test-provider"
...
def test_patient_cannot_read_another_users_ai_passport(tmp_path):
    ...
    with app.app_context():
        ...
        cursor = get_db().execute(
            ...
            """,
            (
                other["id"], "zendoc_ai", "health_records", "test", "test", "low",
                "model", "provider", 0, 1, 1, now_iso(),
            ),
        )
        interaction_id = int(cursor.lastrowid)
        get_db().commit()
