...
def test_personal_baseline_compares_recent_values_to_prior_same_unit_history(tmp_path):
    ...
    response = client.get(
        "/api/v1/health-baseline?metric_type=weight&baseline_days=60&recent_days=7",
...
def test_personal_baseline_fails_closed_when_prior_history_is_insufficient(tmp_path):
    ...
    response = client.get(
        "/api/v1/health-baseline?metric_type=weight&baseline_days=30&recent_days=7",
        headers=_api_headers(client, email),
    )
...
def test_personal_baseline_blocks_cross_patient_idor(tmp_path):
    ...
    response = client.get(
        f"/api/v1/health-baseline?metric_type=weight&patient_id={second_id}",
...
def test_personal_baseline_validates_windows_and_authentication(tmp_path):
    ...
    email = "baseline-window@example.com"
    register_web(client, "patient", email)
    invalid = client.get(
        "/api/v1/health-baseline?metric_type=weight&baseline_days=30&recent_days=30",
        headers=_api_headers(client, email),
    )
