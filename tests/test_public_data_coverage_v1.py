from zendoc.official_connectors import infer_mapping, list_connector_profiles
...
def test_state_drug_controller_connector_is_truthfully_fragmented():
    profiles = {item["source_id"]: item for item in list_connector_profiles()}
    profile = profiles["cdsco_state_drug_control"]
...
def test_owner_coverage_endpoint_is_protected(tmp_path):
    ...
    allowed = client.get("/api/v1/admin/ingestion/coverage")
