...
def test_new_chat_creates_separate_conversations(tmp_path):
    ...
    token = csrf(second_page.data.decode())
    second = client.post("/ai", data={"csrf_token": token, "message": "Help me understand ZENDOC appointments"}, follow_redirects=True)
    assert second.status_code == 200
    with app.app_context():
        rows = get_db().execute("SELECT id FROM ai_conversations WHERE user_id=(SELECT id FROM users WHERE email_normalized=?) ORDER BY id", ("separate-chat@example.com",)).fetchall()
        assert len(rows) == 2
...
def test_doctor_ai_rejects_unrelated_general_task(tmp_path):
    ...
    token = csrf(page.data.decode())
    response = client.post("/ai", data={"csrf_token": token, "mode": "doctor", "message": "Write Python code to sort a list"}, follow_redirects=True)
...
def test_legacy_doctor_form_emergency_still_shows_emergency_state(tmp_path):
    ...
    page = client.get("/ai?new=1")
    token = csrf(page.data.decode())
    response = client.post("/ai", data={"csrf_token": token, "feature": "doctor", "symptoms": "chest pain and trouble breathing"}, follow_redirects=True)
    assert response.status_code == 200
